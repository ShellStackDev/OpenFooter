...
const renderColumns = (grouped: Record<string, OpenFooterLink[]>) => Object.entries(grouped)
  .map(([name, items]) => `<section><h4>${escapeHtml(name)}</h4>${renderLinks(items)}</section>`).join('');
...
export const buildFooterHtml = (config: OpenFooterConfig, links: OpenFooterLink[]): string => {
  ...
  const brand = `<div class="brand"><h3>${escapeHtml(config.brandName ?? 'OpenFooter')}</h3>${config.brandTagline ? `<p class="small">${escapeHtml(config.brandTagline)}</p>` : ''}</div>`;
  ...
  const bodyByLayout: Record<string, string> = {
    ...
    compact: `<div class="layout compact"><nav class="inline-links">${normalized.map((l) => `<a href="${escapeHtml(l.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(l.label)}</a>`).join('')}</nav></div>`,
    full: `<div class="layout full">${brand}<div class="grid columns">${sections}</div></div>`
  };

  return `<div class="wrap"><div class="inner">${bodyByLayout[layout] ?? bodyByLayout.full}<div class="small copyright">© ${new Date().getFullYear()} ${escapeHtml(config.copyrightName ?? config.brandName ?? 'OpenFooter')}${config.showPoweredBy === false ? '' : ` • <a href="https://github.com" target="_blank" rel="noopener noreferrer">Powered by OpenFooter</a>`}</div></div></div>`;
};
