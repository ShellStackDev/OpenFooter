...
`;
	...
	var prefix = "openfooter:";
	var readCache = (key, ttlSeconds) => {
		try {
	...
	//#endregion
	//#region src/utils/google-sheet.ts
	var normalizeGoogleSheetUrl = (url, gid) => {
		let parsed;
		try {
			parsed = new URL(url);
		} catch {
		...
		if (parsed.hostname !== "docs.google.com" || !parsed.pathname.startsWith("/spreadsheets/")) throw new Error("URL must be a Google Sheets docs URL (docs.google.com/spreadsheets/...).");
		if (parsed.pathname.includes("/spreadsheets/d/e/") && parsed.pathname.includes("/pub")) return parsed.toString();
	...
	//#endregion
	//#region src/google-sheet.ts
	var inFlightDataRequests = /* @__PURE__ */ new Map();
	var inFlightTextRequests = /* @__PURE__ */ new Map();
	var withCacheBust = (url) => {
		const u = new URL(url);
		u.searchParams.set("_of_cache_bust", String(Date.now()));
		return u.toString();
	};
	var fetchTextOnce = async (dedupeKeyUrl, requestUrl) => {
	...
	async function getGoogleSheetData(config, forceFresh = false) {
		...
		const task = (async () => {
			...
			try {
				...
				if (/^\s*<!doctype html/i.test(text) || /^\s*<html/i.test(text) || /<title>Google Sheets<\/title>/i.test(text)) throw new Error("OpenFooter could not load this Google Sheet as CSV.\n\nMake sure:\n1. The sheet is shared with \"Anyone with the link can view\", or\n2. The sheet is published to the web, and\n3. The selected sheet tab/gid exists.");
	...
	var SAFE_DATA_IMAGE = /^data:image\/(png|jpeg|jpg|webp|svg\+xml);/i;
	var isSafeUrl = (value) => {
	...
	var normalizeLinks = (links) => links.filter((l) => l.label && isSafeUrl(l.url) && isActiveNow(l)).sort((a, b) => (a.priority ?? 999) - (b.priority ?? 999));
	var groupLinksByCategory = (links) => normalizeLinks(links).reduce((acc, l) => {
		const key = l.category || l.type || "Links";
		(acc[key] ||= []).push(l);
		return acc;
	}, {});
	var renderBrandBlock = (config) => {
		...
		return `<div class="openfooter-brand"><div class="openfooter-brand-main">${config.brandImageUrl && isSafeUrl(config.brandImageUrl) ? `<img class="openfooter-brand-image shape-${shape}" src="${escapeHtml(config.brandImageUrl)}" alt="${escapeHtml(imageAlt)}" loading="lazy" onerror="this.style.display='none'" />` : ""}<div><div class="openfooter-brand-name">${escapeHtml(config.brandName || "")}</div>${config.brandTagline ? `<div class="openfooter-brand-tagline">${escapeHtml(config.brandTagline)}</div>` : ""}${config.brandMessage ? `<div class="openfooter-brand-message">${escapeHtml(config.brandMessage)}</div>` : ""}</div></div></div>`;
	};
	var renderLinks = (items) => `<ul>${items.map((l) => `<li><a href="${escapeHtml(l.url)}" target="_blank" rel="noopener noreferrer">${escapeHtml(l.label)}</a>${l.description ? `<div class="small">${escapeHtml(l.description)}</div>` : ""}</li>`).join("")}</ul>`;
	var renderColumns = (grouped) => Object.entries(grouped).map(([name, items]) => `<section><h4>${escapeHtml(name)}</h4>${renderLinks(items)}</section>`).join("");
	var resolveVisualVars = (config) => {
	...
	var buildFooterHtml = (config, links) => {
		const normalized = normalizeLinks(links);
		const sections = renderColumns(groupLinksByCategory(links));
	...
	var OpenFooterElement = class extends HTMLElement {
		constructor(..._args) {
			super(..._args);
		...
		async refresh(force = true) {
			...
			const data = await getGoogleSheetData({
				...this.config,
				cacheTtlSeconds: this.config.cacheTtlSeconds ?? 0
			}, force);
			const mergedConfig = {
				...data.config,
				...this.config
	...
	var resolveEl = (selectorOrElement) => {
	...
	var OpenFooter = {
		...
		refresh(selectorOrElement) {
			...
			if (target instanceof OpenFooterElement) target.refresh(true);
		}
	};
